...
export const OrganizerLayout: React.FC<OrganizerLayoutProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 antialiased font-sans flex">
      ...
      <aside className="hidden md:flex w-64 fixed inset-y-0 left-0 bg-white border-r border-slate-200/90 flex-col justify-between z-40 overflow-y-auto">
        <div>
          {/* Brand Header */}
          <div className="p-5 border-b border-slate-200/80 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-9 h-9 rounded-xl bg-[#00b894] text-white flex items-center justify-center font-black shadow-md shadow-[#00b894]/20">
              ...
              <div>
                <div className="flex items-center space-x-1.5">
                  ...
                  <span className="text-[10px] font-black bg-[#e6faf5] text-[#00b894] px-1.5 py-0.5 rounded uppercase">2.0</span>
                </div>
      ...
      <main className="flex-1 md:pl-64 flex flex-col min-h-screen bg-[#f8fafc] min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/90 px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-3 flex-1 max-w-md" ref={searchRef}>
            {subpageTitle && onBackToSettingsHub ? (
              <button
                ...
                className="flex items-center space-x-2 text-xs font-bold text-[#00b894] hover:text-[#00a383] bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
              >
              ...
              <div className="relative w-full">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-8 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-hidden focus:border-[#00b894] transition-colors"
                />
                {searchQuery && (
                  <button
                ...
                {isSearchOpen && hasSearchResults && (
                  <div className="absolute left-0 right-0 top-11 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 p-2 max-h-72 overflow-y-auto space-y-2">
                    {/* Events Results */}
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 flex-shrink-0">
            ...
            <div className="relative">
              <button 
                title="Notifications"
                className="relative p-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-600 hover:text-slate-900 transition-colors cursor-pointer"
              >
            ...
            <div className="relative" ref={profileMenuRef}>
              <button
                ...
                className="flex items-center space-x-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl px-3 py-1.5 transition-colors cursor-pointer"
              >
                <div className="w-7 h-7 rounded-lg bg-[#e6faf5] text-[#00b894] font-bold text-xs flex items-center justify-center border border-[#00b894]/30">
                ...
                <div className="text-left hidden sm:block max-w-[130px]">
                  ...
                  <span className="text-[10px] text-[#00b894] font-semibold block uppercase tracking-wider">
                    ORGANIZER
                  </span>
                </div>
              </button>
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 top-12 w-56 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 py-1">
                  <div className="px-3.5 py-2.5 border-b border-slate-100">
                    <p className="text-xs font-bold text-slate-900 truncate">{user?.fullName || 'Organizer User'}</p>
                    <p className="text-[11px] text-slate-500 truncate">{user?.email}</p>
                    <span className="inline-block mt-1 text-[9px] font-black bg-[#e6faf5] text-[#00b894] px-1.5 py-0.5 rounded uppercase">
                      ORGANIZER
                    </span>
                  </div>
                  <button
                  ...
                  <button
                    ...
                    className="w-full text-left px-3.5 py-2 text-xs text-rose-600 hover:bg-rose-50 flex items-center space-x-2 transition-colors cursor-pointer"
                  >
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  isActive ? 'bg-[#00b894] text-white shadow-xs' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                }`}
              >
